...
public class CRS implements Serializable {
    ...
    public boolean makeRendezvous(long national_id, int hospital_id, int section_id, int diploma_id, Date desiredDate) {
        ...
        if (patient == null) {
            throw new IDException("Hasta için yanlış ID: " + national_id);
        }

        Hospital hospital = hospitals.get(hospital_id);
        if (hospital == null) {
            throw new IDException("Hastane için yanlış ID: " + hospital_id);
        }

        Section section = hospital.getSection(section_id);
        if (section == null) {
            throw new IDException("Bölüm için yanlış ID: " + section_id);
        }

        Doctor doctor = section.getDoctor(diploma_id);
        if (doctor == null) {
            throw new IDException("Doktor için yanlış ID: " + diploma_id);
        }
